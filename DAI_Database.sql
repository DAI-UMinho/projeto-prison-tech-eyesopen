...
go 
use [DAI_Database]
create table Tipo (tipo nvarchar(20), cargo bit, primary key (tipo))
...
create table AssocAlerta (id int identity(1,1), idPessoa int, idAlerta int, dataRegisto datetime,  primary key (id), foreign key (idPessoa) references Pessoa (id), foreign key (idAlerta) references Alerta (id) on delete cascade on update cascade)
create table Login (id int, username nvarchar(30), password varBinary(256), primary key (id), foreign key (id) references Pessoa (id))
...
create table Papel (id int identity(1,1), papel nvarchar(40), primary key(id))
create table Privilegio (id int identity(1,1), idPapel int, privilegio nvarchar(100), ativo bit, primary key(id), foreign key (idPapel) references papel(id) on delete cascade on update cascade)
create table AssocPapel (id int identity(1,1), idLogin int, idPapel int,  primary key(id), foreign key (id) references Login (id), foreign key (idPapel) references Papel (id))
