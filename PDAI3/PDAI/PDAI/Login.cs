...
namespace PDAI
{
    class I_Login
    {
        ...
        public I_Login(Form form, int width, int height)
        {
            ...
            lpassword = new Label();
            panelMain.Controls.Add(lpassword);
            ...
            panelMain.Controls.Add(tpassword);
            ...
            panelMain.Controls.Add(iconBox2);
        ...
        private void Hash(object sender, EventArgs e)
        {
            if (tuser.Text != string.Empty && tpassword.Text != string.Empty)
            {
                ...
                {
                    if (Encryption.CheckAccountAccess((byte[])databaseValues[0], Encryption.AccountAccessEncryption(tuser.Text + tpassword.Text + databaseValues[1]))) Logged(EnumConverter<AccessLevel>(databaseValues[1]));
                    else MessageBox.Show("Número de identificação ou password incorreto!");
                }
                else MessageBox.Show("Número de identificação ou password incorreto!");
        ...
        private void Logged(AccessLevel accountAccessLevel)
        {
            switch (accountAccessLevel)
            {
                case AccessLevel.director:
                    break;
                case AccessLevel.recursosHumanos:
                    form.Controls.Clear();
                    I_HumanResources i_administration = new I_HumanResources(form, width, height);
                    break;
                case AccessLevel.secretaria:
                    break;
                case AccessLevel.guardaChefe:
                    break;
                case AccessLevel.guarda:
                    break;
            }
        }
